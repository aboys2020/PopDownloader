...
async function fetchTrackPayload({ aid = fixed.aid, sessionid, track_id }) {
  ...
  console.log(`[track-download] fetchTrackPayload -> url: ${trackV2Url}, sessionidPresent: ${Boolean(sessionid)}, track_id: ${track_id}`)
  ...
  try {
    ...
    // If parsing failed, attempt to read text for diagnostics (if not already)
    if (!err.payload) {
      const text = await trackV2Response.text().catch(() => '')
      err.payload = text
    }
